...
def add_filter_to_group_ui(df):
    if not st.session_state.filter_groups:
        st.sidebar.info("Add a group first")
        return

    with st.sidebar.expander("Add Filter to a Group", expanded=False):
      group_idx = st.selectbox(
          "Select group",
          list(range(len(st.session_state.filter_groups))),
          index=st.session_state.selected_group,
          format_func=lambda x: f"Group {x} (logic: {st.session_state.filter_groups[x]['logic']})"
      )
      ...
      if pd.api.types.is_numeric_dtype(col_data):
          min_val, max_val = st.slider(
              ...
              key=f"range_{group_idx}_{column}"
          )
...
def save_dataset(df):
    ...
    filename = st.text_input("File name", value="filtered_data")
...
def main():
    ...
    if st.session_state.original_df is not None:
        ...
        # Sidebar UI
        add_group_ui()
        add_filter_to_group_ui(df)
        show_filter_groups_ui()

        # Apply filters
        filtered_df = apply_groups(df)
